...
function answerSystem(lens, personaName) {
  ...
  return (
    ...
    '只输出一个 JSON 对象，不要 markdown、不要解释、不要多余的键。必须且只能包含这四个键：kw、plan、why、tag。\n' +
    '示例：{"kw":"小杯少糖","plan":"买小杯三分糖，今天就这一杯。","why":"奖励自己，但别血糖过山车。","tag":"吃喝"}\n' +
    '字段要求：kw=3-6字关键词；plan=15-30字一句具体小建议；why=10-25字温柔理由；tag 从 吃喝/社交/消费/成长/生活 里选一个。'
  );
}
...
function pollSystem() {
  return (
    ...
    '只输出一个 JSON 对象，必须且只能包含键 a（长度为 2 的字符串数组）。\n' +
    '示例：{"a":["去，但只待90分钟","不去，回家回血"]}'
  );
}
